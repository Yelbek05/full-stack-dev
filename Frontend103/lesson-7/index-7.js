...
projectRequestForm.addEventListener("input", (event) => {
    const FormData = {
    ...
    localStorage.setItem("projectRequestForm", JSON.stringify(FormData));
});

window.addEventListener("load", () => {
    const SaveData = JSON.parse(localStorage.getItem("projectRequestForm"));

    if (SaveData){
        projectRequestForm.clientName.value = SaveData.name || "";
        projectRequestForm.email.value = SaveData.email || "";
        projectRequestForm.phone.value = SaveData.phone || "";

        if (SaveData.projectType){
            projectRequestForm.querySelector(`input[name='projectType'][value='${SaveData.projectType}']`).checked = true;
        }
        projectRequestForm.message.value = SaveData.message;
    }
});
...
projectRequestForm.addEventListener("submit", (event) => {
    ...
    if (validateForm()){
        ...
        localStorage.removeItem("projectRequestForm");
